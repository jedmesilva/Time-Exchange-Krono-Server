...
from models.models import User, UserSkill, Balance, TimeSeries, Position, Order, Transaction, MarketPrice, Roll, Roll
...
async def verify_identity(
    ...
    # Em produção: integrar com bureau de verificação de identidade
...
async def emit_series(
    ...
    # Cria posição inicial do emissor
    position = Position(
...
async def place_order(
    ...
    # Verifica se a série existe e está negociável
    ...
    if not series:
        raise HTTPException(status_code=404, detail="Série não encontrada")
    if series.status == SeriesStatus.EXPIRED or series.status == SeriesStatus.LIQUIDATED:
        raise HTTPException(status_code=400, detail="Série não está disponível para negociação")

    # Verifica posição para ASK
...
async def issuer_rolls(issuer_id: str, db: AsyncSession = Depends(get_db)):
    """Histórico de rolagens de um emissor — spreads realizados ao longo do tempo."""
    result = await db.execute(
        select(Roll)
    ...
    return [
        RollResponse(
            ...
            to_series_id=r.to_series_id,
            to_price=float(r.to_price),
            roll_spread=round(float(r.to_price) - float(r.from_price), 4),
            quantity_hours=r.quantity_hours,
...
@rolls_router.get("/me", response_model=list[RollResponse])
async def my_rolls(
    current_user: User = Depends(get_verified_user),
    db: AsyncSession = Depends(get_db),
):
    """Rolagens onde o usuário era titular."""
    result = await db.execute(
        select(Roll)
...
async def get_issuer_prices(issuer_id: str, db: AsyncSession = Depends(get_db)):
    # Último preço de cada série do emissor
    series_result = await db.execute(
...
async def get_issuer_liability(issuer_id: str, db: AsyncSession = Depends(get_db)):
    # Busca todas as séries ACTIVE do emissor
    series_result = await db.execute(
    ...
    for series in series_list:
        # Posições em circulação (exceto o próprio emissor)
...
async def get_yield_curve(issuer_id: str, db: AsyncSession = Depends(get_db)):
    """
    Curva de prazo do emissor — todos os vencimentos abertos ordenados por data.
    Cada ponto inclui o último preço negociado, tipo de contrato e horas em circulação.
    """
    series_result = await db.execute(
        select(TimeSeries)
        .where(
    ...
    for series in series_list:
        ...
        # Horas em circulação (fora da carteira do emissor)
        pos_result = await db.execute(
...
async def check_roll_availability(series_id: str, db: AsyncSession = Depends(get_db)):
    """
    Verifica se há rolagem disponível para uma série.
    Rolagem é possível apenas se o emissor tiver série com vencimento posterior disponível no mercado.
    Retorna o spread de rolagem (contango positivo, backwardation negativo).
    """
    # Busca série atual
    ...
    # Busca próxima série do mesmo emissor com vencimento posterior e status negociável
    ...
    # Último preço da série seguinte
    ...
    # Spread: positivo = contango (próximo mais caro), negativo = backwardation
...
async def get_emitter_profile(issuer_id: str, db: AsyncSession = Depends(get_db)):
    ...
    honor score + liability + composição Fixo/Mercado + carteira própria + curva de prazo.
    """
    # Honor score
    ...
    emitting_continuously = len(active_series) > 1  # tem mais de um vencimento aberto

    composition = EmitterComposition(
        total_series_active=total_active,
        fixed_series=fixed_count,
        market_series=market_count,
        fixed_pct=round((fixed_count / total_active) * 100, 2) if total_active > 0 else 0,
        market_pct=round((market_count / total_active) * 100, 2) if total_active > 0 else 0,
        hours_in_circulation=hours_in_circulation,
...
@rolls_router.get("/issuer/{issuer_id}", response_model=list[RollResponse])
async def issuer_rolls(issuer_id: str, db: AsyncSession = Depends(get_db)):
    """Histórico de rolagens de todas as séries de um emissor."""
    result = await db.execute(
        select(Roll)
        .where(Roll.issuer_id == issuer_id)
        .order_by(Roll.rolled_at.desc())
...
async def my_rolls(
    ...
    """Histórico de rolagens do titular autenticado."""
    result = await db.execute(
        select(Roll)
        .where(Roll.holder_id == current_user.id)
        .order_by(Roll.rolled_at.desc())
...
async def series_rolls(series_id: str, db: AsyncSession = Depends(get_db)):
    """Histórico de rolagens originadas de uma série específica."""
    result = await db.execute(
        select(Roll)
